            ...
            <div>
              <el-form :label-position="'right'" label-width="250px">
                ...
                <el-form-item :label="$t('work.spanUSBcard')">
                  <el-switch v-model="high_setting_form.Span_USBcard">
                  </el-switch>
                </el-form-item>
          ...
          <div class="file">
            <div style="display: flex">
              ...
              <el-progress
                ...
                :format="format"
                style="line-height: 40px; margin-left: 20px; flex: 1"
                :stroke-width="20"
                :color="customColors"
              ></el-progress>
            </div>
            ...
            >
              ...
              >
                <div class="add-btns">
                  <uploader-btn :directory="true">添加文件夹</uploader-btn>
                  <uploader-btn>添加文件</uploader-btn>
                </div>
                <uploader-btn>选择文件</uploader-btn>
								<uploader-btn :directory="true">选择文件夹</uploader-btn>
                <uploader-list class="upload-list"> </uploader-list>
              </uploader-drop>
            </uploader>
           -->
    ...
    <el-row :gutter="20" style="height: 30px; margin-top: 20px">
      <el-col :span="14">
        <div v-if="file_form == 3 && switch_cont">
          ...
          <el-input
            :placeholder="$t('work.pleasePassword')"
            style="width: 25%; float: left; margin-left: 30px"
            show-password
            v-model="p2"
          >
            <template slot="prepend">
          ...
          <el-input
            ...
            style="width: 40%; float: left; margin-left: 30px"
            v-model="zip_name"
          >
            <template slot="prepend">
              {{ $t("work.name") }}
...
export default {
  ...
  methods: {
      ...
      if (this.flag_cont_up) {
        ...
        if (this.switch_tag) {
          ...
          if (this.isNew) {
            ...
            if (this.fileData && !this.$refs.refFile.files[0]) {
              //纯打开，需要上传的是as文件
              let _file = new File(
                [JSON.stringify(this.fileData)],
                this.saveWorkList.taskName,
                { type: "text/plain" }
              );
              data1.append(`file${files_n_incr}`, _file); //标签cs文件
              files_n_incr++;
            } else if (this.fileData && this.$refs.refFile.files[0]) {
              //打开后，又重新选择了soon文件
              data1.append(`file${files_n_incr}`, this.$refs.refFile.files[0]); //标签cs文件
              files_n_incr++;
            } else {
    ...
    async upload() {
      ...
      */
      ...
      if (this.switch_cont) {
        //开启了左边内容则先左边
        if (
          !this.high_setting_form.Span_USBcard &&
          this.size >
        ...
        */
        ...
        if (this.high_setting_form.localfiles) {
          ...
          if (t) {
            ...
            this.$refs.files.copyPath =
              this.shareDisk + "\\" + this.upload_disk + "\\data\\";
    ...
    submit() {
      ...
      if (this.file_form == 0 || this.file_form == 1 || this.file_form == 4) {
        //文件和文件夹
        let file_path = [];
        for (let i in this.$refs.files.filesList) {
